...
17.06.15		1.2			CH02			Nachiket Bapat				7-Segment					Display of EEPROM count on 7-segment for debugging purpose
18.06.15		1.3			CH03			Subhajit Pande				Map function				Addition of Map function and negative value check
19.06.15		1.4			CH04			Subhajit Pande				Button tuning				Button release tuning
*/
...
#include "Seven_Segment.h" //CH02 - header file for 7-segment
...
uint8_t keyPressAndRelease(uint8_t button)
{
	...
	if(!(PINC & (1<<button)))// button pressed
	{
		...
		if(!(PINC & (1<<button)))
		{
			...
			while(!(PINC & (1<<button))); // CH04- wait for button release
			return 1;
		}
		else
			return 0;
...
/* CH03 begins*/
/*Function to map the average road variation into scale of 0-5
return- Road grade on a scale 0-5*/
